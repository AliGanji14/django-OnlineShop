...
#         'callback_url': request.build_absolute_uri(reverse('payment:payment_call_back')),
#     }
#
...
def payment_process_sandbox(request):
    ...
    request_data = {
        ...
        'Amount': rial_total_price,
        'Description': f'#{order.id}: {order.user.first_name} {order.user.last_name}',
        'CallbackURL': request.build_absolute_uri(reverse('payment:payment_call_back')),
    }
...
def payment_call_back_sandbox(request):
    payment_authority = request.GET.get('Authority')
    payment_status = request.GET.get('Status')
